...
function renderPlaymaking() {
  ...
  const types = [
    { key: 'pnr', label: 'Pick & Roll', color: '#F57B20' },
    { key: 'postUp', label: 'Post-Up', color: '#C2790A' },
    { key: 'spotUp', label: 'Spot-Up', color: '#2FAE72' },
    { key: 'transition', label: 'Transition', color: '#FFA655' },
  ];
...
function courtDiagramSVG() {
  ...
  const colors = ['#F57B20', '#FFA655', '#FFA655', '#E8B54D', '#57606F'];
  const dots = COURT_POSITIONS.map((p, i) => `
    <g>
      <circle cx="${p.x}" cy="${H - p.y}" r="16" fill="${colors[i % colors.length]}" stroke="#14161A" stroke-width="2"/>
      <text x="${p.x}" y="${H - p.y + 5}" text-anchor="middle" font-family="Oswald, sans-serif"
        font-weight="700" font-size="14" fill="#14161A">${p.num}</text>
    </g>`).join('');

  return `
  <svg width="100%" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    <rect x="0" y="0" width="${W}" height="${H}" fill="#1E2126" />
    <!-- court boundary -->
    <rect x="10" y="10" width="${W-20}" height="${H-20}" fill="none" stroke="#F57B20" stroke-width="3"/>
    <!-- half-court line -->
    <line x1="10" y1="10" x2="${W-10}" y2="10" stroke="#F57B20" stroke-width="3"/>
    <circle cx="${W/2}" cy="10" r="40" fill="none" stroke="#F57B20" stroke-width="2"/>
    <!-- key / lane -->
    <rect x="${hoopX-80}" y="${hoopY-190}" width="160" height="190" fill="none" stroke="#F57B20" stroke-width="2"/>
    <!-- free throw circle -->
    <circle cx="${hoopX}" cy="${hoopY-190}" r="45" fill="none" stroke="#F57B20" stroke-width="2" stroke-dasharray="4 4"/>
    <!-- three point arc (approximate) -->
    <path d="M 35 ${hoopY-10} A 220 220 0 0 1 ${W-35} ${hoopY-10}" fill="none" stroke="#F57B20" stroke-width="2"/>
    <line x1="35" y1="${hoopY-10}" x2="35" y2="${H-10}" stroke="#F57B20" stroke-width="2"/>
    <line x1="${W-35}" y1="${hoopY-10}" x2="${W-35}" y2="${H-10}" stroke="#F57B20" stroke-width="2"/>
    <!-- backboard & rim -->
    <rect x="${hoopX-25}" y="${hoopY+6}" width="50" height="4" fill="#F1EFE9"/>
    <circle cx="${hoopX}" cy="${hoopY}" r="8" fill="none" stroke="#FFA655" stroke-width="3"/>
    ${dots}
  </svg>`;
}
