...
This is a static site with no backend, so it can't poll a live stats API by itself (WNBA/NBA's stats endpoints aren't meant for public, unauthenticated, cross-origin use from a browser, and Basketball-Reference doesn't offer a public JSON API). 
